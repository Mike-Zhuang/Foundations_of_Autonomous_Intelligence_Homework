...
| `--ny` | 200 | 网格高度（行数） |
| `--nx` | 800 | 网格宽度（列数） |
| `--tau` | 0.56 | 碰撞弛豫时间，影响粘性系数 $\nu = \frac{\tau - 0.5}{3}$ |
| `--u-in` | 0.08 | 左侧入口流速（格子单位） |
| `--steps-per-frame` | 6 | 每帧动画执行的 LBM 迭代次数 |
| `--obstacle-mode` | `circle` | 障碍物初始化模式：`circle`（圆柱）或 `image`（读取图片） |
| `--obstacle-image` | — | `image` 模式下的图片路径（黑色像素视为固体） |
| `--non-periodic-y` | 否 | 开启后上下边界使用弹回（Bounce-back）代替周期性边界 |
| `--frames` | 5000 | 交互模式下总动画帧数 |
| `--interval` | 30 | 动画帧间隔（ms） |
| `--export` | — | 导出路径（`.mp4` 或 `.gif`），指定后进入无交互批量渲染模式 |
| `--export-frames` | 600 | 导出模式下的总渲染帧数 |
| `--fps` | 30 | 导出时的帧率 |
...
程序启动后，会弹出 Matplotlib 可视化界面，分为三个面板：
- **左面板**：速度幅值热力图（`inferno` 色图，自动动态上限）
- **中面板**：涡量场（`seismic` 双色图）或迷宫流体透过图，叠加白色障碍物轮廓与 Q 准则涡核高亮
- **右面板**：升力系数历史折线图，实时追踪 3 种方法的 $C_l$ 时序（最近 900 帧）

界面底部设有专用信息栏，实时显示仿真步数、tau、u_in、雷诺数、画笔状态及三种力估算结果。当光标在画布内时，你可使用以下快捷键进行实时控制：
...
| `5` | **正方形障碍物 (Square)**：经典钝体，产生强规则卡门涡街 |
| `6` | **椭圆障碍物 (Ellipse)**：长轴垂直于流向（~2:1），产生宽尾迹涡街 |
| `7` | **三角形障碍物 (Triangle)**：等边三角形，顶点朝向来流方向 |
| `8` | **平板障碍物 (Flat Plate)**：垂直于流向的薄板，产生剧烈涡脱落 |
| `9` | **菱形障碍物 (Diamond)**：正方形旋转 45°，分离模式与方形不同 |
| `I` | **导入图像 (Image)** 读取并导入同目录下的 `obstacle.png` 作为流体迷宫 |
| `M` | 切屏：在 **涡量计算视图 (Vorticity)** 与 **流体迷宫 (Maze)** 视图之间切换 |
| `K` | **卡门涡街快速演示（Karman Demo）**：自动恢复圆柱障碍物，预热 220 步并施加初始扰动，一键触发涡列 |
| `N` | 生成 **NACA 0012 翼型** (自带 8 度攻角，演示动态升力流场) |
| `P` | 开关 **初始扰动（Initial Perturbation）**：控制是否在重置时注入反对称扰动以提前触发涡脱落 |
| `V` | 开关 **涡核叠加层（Vortex-Core Overlay）**：基于 Q 准则高亮显示旋涡核心区域 |
| `A` | 切换涡量色标模式：**动态自适应（Dynamic）** 与 **固定范围（Fixed）** |
...
3. 一旦接触右侧边界，立即回溯生成一条具有最少步数的最佳通路，路径加宽 1 像素后写入 `maze_path_mask`。
4. 路径不立即全显：在每帧 `update` 中检测流速超过阈值（$0.05 \cdot u_{in}$）的路径格子，**逐步高亮（Progressive Reveal）**，实现流体"点亮"通路的视觉效果。

### 4. 气动力三重估算体系
对障碍物所受气动力，程序并行维护三套独立估算方法，并在信息栏与右侧面板实时可视化三条 $C_l$ 时序曲线：

| 方法 | 实现函数 | 原理简述 |
| --- | --- | --- |
| **动量交换法 (MEM)** | `estimate_momentum_exchange_force` | 统计流体→固体边界链接处被反弹的分布函数动量之和，对每帧积累后取均值 |
| **压力积分法 (Pressure)** | `estimate_pressure_force` | 用梯度运算求障碍物边界法向量，将局部压力 $p=\rho/3$ 沿法向积分 |
| **库塔-茹柯夫斯基环量法 (Gamma)** | `estimate_circulation_lift` | 沿绕障碍物的矩形围道积分速度，估算环量 $\Gamma$，再由 $L=\rho_\infty U_\infty \Gamma$ 换算升力 |

### 5. Q 准则涡核检测 (`compute_q_criterion`)
逐帧计算速度梯度张量的 Q 值（旋转项平方减应变率项之半），取场内 top-0.7% 高值格子作为涡核区域，以半透明白色叠加层实时标注于左、中双面板（可用 `V` 键开关，`A` 键控制涡量色标范围）。

### 6. 升力系数历史面板（第三面板）
`build_figure` 在右侧额外创建 `ax3` 折线图，同步追踪并显示最近 900 帧的三条 $C_l$ 时序（MEM 绿线、Pressure 黄线、Gamma 蓝线），为气动力收敛性和周期性涡脱落提供直观的时域视图。
